    ...
    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="text-right">
          <div className="text-sm font-extrabold text-blue-600">{progressPercent}% Completed</div>
          <div className="text-[11px] text-slate-400">{completedCount} of {milestones.length} milestones</div>
        </div>
      </div>
        ...
        {milestones.map((m, idx) => (
          ...
          >
            ...
            {!m.completed && m.actionUrl && (
              <Link
                href={m.actionUrl}
                className="shrink-0 px-2.5 py-1 text-[11px] font-semibold rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition flex items-center gap-1 shadow-sm"
              >
      ...
      {progressPercent === 100 && (
        <div className="bg-gradient-to-r from-amber-500 to-amber-600 text-slate-950 p-4 rounded-xl font-medium text-xs text-center flex flex-col items-center gap-2 shadow-md">
          ...
          <button
            ...
            className="mt-1 px-3 py-1 bg-slate-950 text-amber-300 font-bold rounded-lg text-xs hover:bg-slate-900 transition"
          >
            Celebrate Again! 🎊
          </button>
        </div>
