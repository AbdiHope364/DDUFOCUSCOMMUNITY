    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm space-y-4">
        <div className="flex items-center justify-between text-xs pb-3 border-b border-slate-100 dark:border-slate-800">
          ...
          <span className={`px-2.5 py-0.5 rounded-full font-bold text-[10px] ${todayData.isFallback ? 'bg-amber-100 dark:bg-amber-950/80 text-amber-900 dark:text-amber-300' : 'bg-emerald-100 dark:bg-emerald-950/80 text-emerald-800 dark:text-emerald-300'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
          <div className="space-y-1">
            <div className="text-[10px] uppercase text-slate-400 dark:text-slate-500 font-bold">Active Title & Reference:</div>
            <div className="font-bold text-sm text-slate-900 dark:text-white">{todayData.word.title}</div>
            <div className="text-blue-600 dark:text-blue-400 font-semibold">{todayData.word.scriptureReference}</div>
            <blockquote className="text-slate-600 dark:text-slate-300 italic bg-slate-50 dark:bg-slate-800/60 p-2.5 rounded-xl border border-slate-100 dark:border-slate-700 mt-2">
              &ldquo;{todayData.word.verseText}&rdquo;
            </blockquote>
          </div>

          <div className="space-y-2 bg-slate-50 dark:bg-slate-800/60 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 text-slate-600 dark:text-slate-300">
            <div className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
              <Clock className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" /> Auto-Publish Schedule Details
            </div>
            <p className="text-[11px] leading-relaxed">
              System cron runs every midnight at <span className="font-mono font-bold text-slate-800 dark:text-slate-200">00:00:00 East Africa Time (UTC+3)</span>.
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm space-y-4">
        <div className="flex items-center justify-between text-xs pb-2 border-b border-slate-100 dark:border-slate-800">
          ...
          <span className="text-slate-400 dark:text-slate-500 text-[11px]">365-Day Buffer</span>
        </div>
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-800">
          {store.dailyWords.map((word) => (
            <div
              key={word.id}
              className="py-3.5 flex flex-wrap items-center justify-between gap-4 text-xs hover:bg-slate-50/60 dark:hover:bg-slate-800/60 p-2 rounded-xl transition"
            >
              <div className="space-y-1 max-w-lg">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-slate-600 dark:text-slate-400 font-bold bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded text-[11px]">
                    {word.publishDate}
                  </span>
                  <span className="font-bold text-slate-900 dark:text-white">{word.title}</span>
                  <span
                ...
                <div className="text-slate-500 dark:text-slate-400 font-medium">{word.scriptureReference} — &ldquo;{word.verseText}&rdquo;</div>
              </div>
      ...
      {showScheduleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 rounded-3xl max-w-xl w-full p-6 sm:p-8 shadow-2xl relative border border-slate-200 dark:border-slate-800 max-h-[90vh] overflow-y-auto space-y-4">
            ...
            <form onSubmit={handleSchedule} className="space-y-3 text-xs">
              ...
              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Title / Devotional Theme *</label>
                <input
                  type="text"
                  required
                  value={title}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="sm:col-span-2">
                  <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Bible Verse Text *</label>
                  <textarea
                    rows={2}
                    required
                    value={verseText}
                ...
                <div>
                  <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Scripture Reference *</label>
                  <input
                    type="text"
                    required
              ...
              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Spiritual Reflection</label>
                <textarea
                  rows={3}
                  value={reflection}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Daily Student Prayer</label>
                  <textarea
                    rows={2}
                    value={prayer}
                ...
                <div>
                  <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Action Challenge</label>
                  <textarea
                    rows={2}
                    value={challenge}
