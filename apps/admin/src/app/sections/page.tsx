    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm space-y-4">
        <div className="flex items-center justify-between text-xs pb-3 border-b border-slate-100 dark:border-slate-800">
          ...
          <span className="text-slate-400 dark:text-slate-500 text-[11px]">Database-Managed</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {store.sections.map((sec) => (
            <div
              key={sec.id}
              className="p-5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/60 flex flex-col justify-between space-y-3"
            >
              <div className="space-y-1">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-bold text-base text-slate-900 dark:text-white">{sec.name}</span>
                  <span className="px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-950/80 text-blue-800 dark:text-blue-300 text-[10px] font-bold">
                    {sec.memberCount} members
                  </span>
                </div>
                <p className="text-xs text-slate-600 dark:text-slate-300 line-clamp-2">{sec.description}</p>
              </div>

              <div className="pt-2 border-t border-slate-200 dark:border-slate-700 text-xs text-slate-500 dark:text-slate-400 space-y-1">
                <div><span className="font-semibold text-slate-700 dark:text-slate-300">Leader:</span> {sec.leaderName || 'Unassigned'}</div>
                <div><span className="font-semibold text-slate-700 dark:text-slate-300">Schedule:</span> {sec.meetingSchedule}</div>
                <div><span className="font-semibold text-slate-700 dark:text-slate-300">Venue:</span> {sec.meetingLocation}</div>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 rounded-3xl max-w-xl w-full p-6 sm:p-8 shadow-2xl relative border border-slate-200 dark:border-slate-800 max-h-[90vh] overflow-y-auto space-y-4">
            ...
            <form onSubmit={handleCreate} className="space-y-3 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">URL Slug *</label>
                  <input
                    type="text"
                    required
                    value={slug}
              ...
              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Ministry Description *</label>
                <textarea
                  rows={2}
                  required
                  value={description}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Assigned Section Leader</label>
                  <input
                    type="text"
                    value={leaderName}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Meeting Schedule</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Meeting Location</label>
                  <input
                    type="text"
